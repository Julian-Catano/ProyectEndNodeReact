...
export const fetchLogin = createAsyncThunk('auth/login', async(formData) => {
    try {
        const response = await axios.post(`${import.meta.env.VITE_URL_SERVER}api/auth/auth-user`, formData)
...
export const fetchValidateToken = createAsyncThunk('auth/validateToken', async() => {
    try {
        const response = await axios.post(`${import.meta.env.VITE_URL_SERVER}api/auth/validate-user`, {}, {
            headers: {
